...
class IGround : public Ground {
    public:
        ...
        int last_count = 0;
        bool preplanned = true;
        std::vector<int> current_deliveries;
        int strategy_sort = 0; // 0: None, 1: Azimuthal, 2: Distance
        int strategy_assign = 0; // 0: Round Robin, 1: Block Distribution, 2: Dynamic
};

IGround::IGround(const rclcpp::NodeOptions & options) : Ground(options) {
    marker_once("Inherited Ground (IGround) node initialized!");
    marker_once("This is RQ4 Evaluation Ground Node!");
    ...
    // read sort, assign parameters from ros2 launch
    this->declare_parameter<std::string>("str_sort", "0");
    // marker("DEBUG");
    this->declare_parameter<std::string>("str_assign", "0");
    this->strategy_sort = std::stoi(this->get_parameter("str_sort").as_string());
    this->strategy_assign = std::stoi(this->get_parameter("str_assign").as_string());
    
    marker("Sorting strategy: %d, Assignment strategy: %d", 
                this->strategy_sort, this->strategy_assign);
    this->preplanned = (this->strategy_assign == 0 || this->strategy_assign == 1);
}
...
void IGround::prep_once() {
    ...
    // // plan X: default: no sort.
    if (this->strategy_sort == 0) {
        this->marker(100, "No sorting strategy applied.");
        // do nothing
    } else if (this->strategy_sort == 1) {
        ...
        this->marker(100, "Sorted delivery points by azimuthal angle.");
    } else if (this->strategy_sort == 2) {
        ...
        // // plan C: deliver from farthest to closest: not used
        // std::sort(this->deliver_list.begin(), this->deliver_list.end(), [](const delivery& a, const delivery& b) {
        //     return std::sqrt(a.north * a.north + a.east * a.east) > std::sqrt(b.north * b.north + b.east * b.east);
        // });
        this->marker(100, "Sorted delivery points by distance from origin.");
    }
    

    if (preplanned){
        // assignment strategy handling
        if (this->strategy_assign == 0) {
            // // plan B: round-robin randomly sorted deliveries
            for (int i = 0; i < this->deliver_list.size(); i++) {
                int drone_id = i % this->total_swarm_size; // Assign deliveries in round-robin fashion
                this->drone_deliveries[drone_id].push_back(i);
            }
            this->marker(0, "Round-robin assignment of deliveries to drones.");
        } else {
            // // // // // default: bulk assign deliveries to drones // // // // //
            // (each drone gets deliver_list.size() / swarm_size deliveries at once)
            for (int i = 0; i < this->deliver_list.size(); i++) {
                int drone_id = i / (this->deliver_list.size() / this->total_swarm_size);
                // should we reassign package id?
                this->drone_deliveries[drone_id].push_back(i);
            }
            this->marker(0, "Bulk assignment of deliveries to drones.");
        }
