...
def launch_setup(context: LaunchContext):
    ...
    get_logger().info(f"DBG: Number of drone nodes to launch: {num_drones_val}")

    for i in range(num_drones_val):
        drone_remaps = []
        if i == target_id:
            get_logger().info(f"DBG: Remapping topic for drone_{i}")
            topics_to_remap = ['task_cmd', 'heartbeat', 'prox_alert'] # 'report'
            for topic in topics_to_remap:
                original = f"/drone_{i}/{topic}"
                faulty   = f"/drone_{i}/{topic}_faulty"
                drone_remaps.append((original, faulty))
                print(f"DBG: Remapping topic for drone_{i}: {original} -> {faulty}")
